...
@Component({
  selector: 'app-why-smart',
  templateUrl: './why-smart.component.html',
  styleUrls: ['./why-smart.component.scss']
})
export class WhySmartComponent implements OnInit, OnDestroy {
  ...
  */
  setSubscriptions() {
    ...
    this._subscription.add(
      this._store.select('api').subscribe((state) => {
        ...
        this.fullData = state.arrayWhy.apiArray;
        if (this.apiConWhy !== undefined) {
  ...
  getDataAPI(lang: string) {
    if (!this.apiConWhy) {
      this._apiJSONService.getJSON(lang, 'whysmart', true).subscribe(
        (resp: any) => {
          this.data = resp;
          if (this.data !== undefined) {
            this._reduxService.setArray('whysmart', this.data, lang);
  ...
  getDataArray(array: any) {
    if (Object.keys(array).length > 0) {
      ...
      this.dataBenefits = this.data.benefits;
