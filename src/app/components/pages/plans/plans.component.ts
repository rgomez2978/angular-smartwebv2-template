...
export class PlansComponent implements OnInit, OnDestroy {
  ...
  */
  setSubscriptions() {
    ...
    this._subscription.add(
      this._store.select('api').subscribe((state) => {
        ...
        this.fullData = state.arrayPlans.apiArray;
        if (this.apiConPlans !== undefined) {
  ...
  getDataAPI(lang: string) {
    if (!this.apiConPlans) {
      this._apiJSONService.getJSON(lang, 'plans', true).subscribe(
        (resp: any) => {
          this.data = resp;
          if (this.data !== undefined) {
            this._reduxService.setArray('plans', this.data, lang);
