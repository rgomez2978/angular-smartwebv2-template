...
export class ContactusComponent implements OnInit, OnDestroy {
  ...
  */
  setSubscriptions() {
    ...
    this._subscription.add(
      this._store.select('api').subscribe((state) => {
        ...
        this.fullData = state.arrayContacts.apiArray;
        if (this.apiConContacts !== undefined) {
  ...
  getDataAPI(lang: string) {
    if (!this.apiConContacts) {
      this._apiJSONService.getJSON(lang, 'contacts', true).subscribe(
        (resp: any) => {
          this.data = resp;
          if (this.data !== undefined) {
            this._reduxService.setArray('contacts', this.data, lang);
